...
class BaseLightningExperiment(BaseExperiment):
    ...
    def __init__(
        ...
        self.deterministic = "warning" if root_cfg.seed is not None else None
        self.trainer = None
        self.any_training_done = False  # to check if training has been done
    ...
    def training(self) -> None:
        ...
        if self.cfg.training.early_stopping.enabled:
            callbacks.append(
                LazyEarlyStopping(
                    lazy=self.cfg.training.checkpointing.lazy,
                    monitor=self.cfg.training.checkpointing.monitor,
                    mode=self.cfg.training.checkpointing.mode,
                    patience=self.cfg.training.early_stopping.patience,
                )
            )

        callbacks.append(GlobalProgressBar())
        ...
        if self.cfg.validation.test_during_training:
            val_dataloaders.append(self._build_test_loader())
    ...
    def test(self) -> None:
        ...
        self.trainer.test(
            ...
            ckpt_path=(
                self.ckpt_path
                if (self.ckpt_path is not None and not self.any_training_done)
                else "best"  # only test the loaded checkpoint if there was no further training
            ),
        )
